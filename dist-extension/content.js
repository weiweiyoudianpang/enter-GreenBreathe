function T(){const e=document.createElement("div");return e.id="green-breathe-notification-root",document.body.appendChild(e),e}function k(){if(document.fullscreenElement)return console.log("[GreenBreathe] Delayed: Fullscreen detected"),!0;const e=document.activeElement;if(e&&(e.tagName==="INPUT"||e.tagName==="TEXTAREA"||e.getAttribute("contenteditable")==="true"))return console.log("[GreenBreathe] Delayed: User is typing"),!0;const i=document.querySelectorAll("video");for(const o of i){const r=o.getBoundingClientRect(),c=r.width>200&&r.height>150;if(!o.paused&&o.currentTime>0&&c)return console.log("[GreenBreathe] Delayed: Video playing"),!0}const t=document.querySelectorAll("audio");for(const o of t)if(!o.paused&&o.currentTime>0)return console.log("[GreenBreathe] Delayed: Audio playing"),!0;return navigator.mediaDevices&&document.querySelector("video[autoplay]")?(console.log("[GreenBreathe] Delayed: Video call detected"),!0):document.querySelectorAll('form[data-submitting="true"]').length>0?(console.log("[GreenBreathe] Delayed: Form submitting"),!0):(console.log("[GreenBreathe] ✓ Safe to show notification"),!1)}let f=0,p=0;document.addEventListener("mousemove",()=>{f=Date.now()},{passive:!0});document.addEventListener("scroll",()=>{p=Date.now()},{passive:!0});function S(){const e=Date.now();return e-f<5e3||e-p<5e3}async function l(e,i=0){if(k()&&i<3){console.log(`[GreenBreathe] Retry attempt ${i+1}/3 after 30s`),setTimeout(()=>l(e,i+1),3e4);return}if(S()&&i===0){console.log("[GreenBreathe] User actively interacting, delaying 10s"),setTimeout(()=>l(e,i),1e4);return}const a=(await chrome.storage.local.get("userProfile")).userProfile?.notificationPosition||"top_right",n=await z(e,a);let o=document.getElementById("green-breathe-notification-root");o||(o=T()),o.innerHTML="",o.appendChild(n),setTimeout(()=>{const r=n.shadowRoot?.querySelector(".notification-card");r&&r.classList.add("show")},100),setTimeout(()=>{g(n)},8e3)}async function z(e,i){const t=document.createElement("div");t.className=`green-breathe-notification ${i}`;const a=t.attachShadow({mode:"open"}),n=await chrome.storage.local.get("userProfile"),o=n.userProfile?.mbtiType||"INFP",r=n.userProfile?.cardSize||"medium",c={small:{width:960,height:570},medium:{width:1280,height:760},large:{width:1600,height:950}},{width:h,height:b}=c[r],x=o.includes("T"),y=o.includes("N");let s="";x?s=e.instruction.mbtiAdaptation.T:y?s=e.instruction.mbtiAdaptation.N:s=e.instruction.mbtiAdaptation.F;const d=["了解啦","谢谢关心","OK","收到","这就去"],v=d[Math.floor(Math.random()*d.length)],u=["https://grazia-prod.oss-ap-southeast-1.aliyuncs.com/resources/uid_100003000/a4ec.png","https://grazia-prod.oss-ap-southeast-1.aliyuncs.com/resources/uid_100003000/f5db.png","https://grazia-prod.oss-ap-southeast-1.aliyuncs.com/resources/uid_100003000/e7ab.png"],w=u[Math.floor(Math.random()*u.length)];return a.innerHTML=`
  ...
  `,a.querySelector(".action-dismiss")?.addEventListener("click",()=>{N("dismissed",e.taskType),g(t)}),t}function g(e){const t=e.shadowRoot?.querySelector(".notification-card");t&&(t.style.animation="none",t.style.transition="all 1.5s cubic-bezier(0.22, 1, 0.36, 1)",t.style.opacity="0",t.style.filter="blur(10px)",t.style.transform="scale(0.95)"),setTimeout(()=>{e.remove()},2e3)}async function N(e,i){try{const t={timestamp:Date.now(),action:e,taskType:i},n=(await chrome.storage.local.get("interactionLog")).interactionLog||[];n.push(t),n.length>100&&n.splice(0,n.length-100),await chrome.storage.local.set({interactionLog:n})}catch(t){console.error("Error logging interaction:",t)}}chrome.runtime.onMessage.addListener((e,i,t)=>{e.type==="SHOW_NOTIFICATION"&&(l(e.data),t({success:!0}))});const m=document.createElement("style");m.textContent=`
...
`;document.head.appendChild(m);console.log("GreenBreathe Content Script Loaded");
